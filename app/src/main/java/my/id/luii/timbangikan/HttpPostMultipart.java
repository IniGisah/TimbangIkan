...
public class HttpPostMultipart {
    ...
    /**
     * Adds a form integer field to the request
     *
     * @param name  field name
     * @param value field value
     */
    public void addFormIntField(String name, String value) {
        writer.append("--" + boundary).append(LINE);
        writer.append("Content-Disposition: form-data; name=\"" + name + "\"").append(LINE);
        //writer.append("Content-Type: text/plain; charset=" + charset).append(LINE);
        writer.append(LINE);
    ...
    public String finish() throws IOException {
        ...
        if (status == HttpURLConnection.HTTP_OK) {
            ...
            response = result.toString(this.charset);
            httpConn.disconnect();
            throw new IOException("Server returned non-OK status: " + response);
        }
        return response;
    }
}